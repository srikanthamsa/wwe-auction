    ...
    <div style={{ position:'relative', zIndex:1, minHeight:'100vh', fontFamily:'Outfit, sans-serif' }}>
      ...
      <div style={{ maxWidth:700, margin:'0 auto', padding:'2.5rem 1.5rem 4rem' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'2.5rem', animation:'fadeUp 0.5s ease' }}>
          <div style={{ fontSize:'0.75rem', letterSpacing:'0.5em', color:'rgba(30,41,59,0.45)', marginBottom:'0.5rem', textTransform:'uppercase' }}>Auction Complete</div>
          ...
          <div style={{ fontSize:'0.85rem', color:'rgba(30,41,59,0.35)', letterSpacing:'0.2em', marginTop:'0.3rem' }}>{totalSold} superstars sold</div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'0.7rem', marginBottom:'1.75rem', animation:'fadeUp 0.55s ease' }}>
          {[
            ...
            <div key={label} style={{ padding:'0.9rem', background:'rgba(15,23,42,0.04)', boxShadow:'inset 0 0 0 1px rgba(15,23,42,0.07)', borderRadius:14, textAlign:'center' }}>
              <div style={{ fontFamily:'Bebas Neue', fontSize:'1.6rem', color:'#a78bfa', letterSpacing:'0.04em' }}>{val}</div>
              <div style={{ fontSize:'0.58rem', color:'rgba(30,41,59,0.4)', letterSpacing:'0.2em', textTransform:'uppercase', marginTop:2 }}>{label}</div>
            </div>
          ))}
        </div>
            ...
            return (
              <div key={s.name}>
                <div className="stand-row"
                  ...
                  style={{ display:'flex', alignItems:'center', gap:'0.75rem', padding:'1rem 1.1rem', background: isFirst ? `rgba(${rgb},0.1)` : isMe ? `rgba(${rgb},0.07)` : 'rgba(15,23,42,0.04)', boxShadow: isFirst ? `inset 0 0 0 1px rgba(${rgb},0.35), 0 0 24px rgba(${rgb},0.1)` : isMe ? `inset 0 0 0 1px rgba(${rgb},0.2)` : 'inset 0 0 0 1px rgba(15,23,42,0.06)', borderRadius: open ? '14px 14px 0 0' : 14 }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:'0.68rem', color:'rgba(30,41,59,0.4)', letterSpacing:'0.08em' }}>
                  ...
                  <div style={{ textAlign:'right' }}>
                    ...
                    <div style={{ fontSize:'0.58rem', color:'rgba(30,41,59,0.35)', letterSpacing:'0.1em' }}>SPENT</div>
                  </div>
                  <div style={{ fontSize:'0.75rem', color:'rgba(30,41,59,0.3)', marginLeft:'0.25rem' }}>{open ? '▲' : '▼'}</div>
                </div>
                  ...
                  <div style={{ background:'rgba(255,255,255,0.55)', boxShadow:`inset 0 0 0 1px rgba(${rgb},0.1)`, borderTop:'none', borderRadius:'0 0 14px 14px', padding:'0.85rem' }}>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'0.4rem', marginBottom:'0.7rem' }}>
                      {[
                        ...
                        <div key={label} style={{ textAlign:'center', padding:'0.35rem', background:'rgba(15,23,42,0.04)', borderRadius:9 }}>
                          <div style={{ fontFamily:'Bebas Neue', fontSize:'1.05rem', color:col }}>{val}</div>
                          <div style={{ fontSize:'0.5rem', color:'rgba(30,41,59,0.4)', textTransform:'uppercase', letterSpacing:'0.1em' }}>{label}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{ display:'flex', flexDirection:'column', gap:'0.2rem' }}>
                      {s.roster.sort((a, b) => b.ovr - a.ovr).map((star, j) => {
                        ...
                        return (
                          <div key={j} style={{ display:'flex', alignItems:'center', padding:'0.32rem 0.55rem', background:'rgba(15,23,42,0.03)', borderRadius:8, gap:'0.5rem' }}>
                            <div style={{ fontFamily:'Bebas Neue', fontSize:'0.82rem', color:tc, minWidth:'2rem' }}>{star.ovr}</div>
                            <div style={{ flex:1, fontSize:'0.88rem', fontWeight:600, color:'rgba(15,23,42,0.8)' }}>{star.superstar}</div>
                            <div style={{ fontFamily:'Bebas Neue', fontSize:'0.78rem', color:'rgba(30,41,59,0.45)' }}>₹{star.price>=1000?`${(star.price/1000).toFixed(1)}k`:star.price}</div>
                          </div>
                        )
                      })}
                    </div>
          ...
          <div style={{ textAlign:'center', marginTop:'1rem' }}>
            {!confirmReset ? (
              ...
              <div style={{ display:'flex', gap:'0.75rem', justifyContent:'center', alignItems:'center' }}>
                <span style={{ fontSize:'0.85rem', color:'rgba(30,41,59,0.5)', letterSpacing:'0.1em', fontFamily:'Outfit' }}>Resets everything.</span>
                ...
                <button onClick={() => setConfirmReset(false)} style={{ background:'none', border:'none', fontSize:'0.82rem', color:'rgba(30,41,59,0.25)', cursor:'pointer', fontFamily:'Outfit' }}>Cancel</button>
              </div>
            )}
          </div>
        )}
...
function Divider({ label }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'0.6rem', marginBottom:'0.9rem', fontFamily:'Outfit', fontSize:'0.6rem', letterSpacing:'0.38em', color:'rgba(30,41,59,0.4)', textTransform:'uppercase' }}>
